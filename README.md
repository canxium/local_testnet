...
These scripts allow for running a small local testnet with multiple beacon nodes and validator clients and a canxium execution client.
This setup can be useful for testing and development.

## Requirements

The scripts require `lcli`, `lighthouse`, `canxium`, `bootnode` to be installed on `PATH` (run `echo $PATH` to view all `PATH` directories).
...
This will build `lighthouse` and `lcli`.

For `bootnode`, go to [geth website](https://geth.ethereum.org/downloads) and download the `Geth & Tools`. For example, to download and extract `Geth & Tools 1.13.1`:
...
After extraction, copy `bootnode` to the `PATH`. A typical directory is `/usr/local/bin`.
...
For `canxium`, go to [canxium website](https://github.com/canxium/go-canxium) and clone the repo and build canxium:

```bash
cd ~
git clone https://github.com/canxium/go-canxium.git
cd go-canxium && make canxium
```

After extraction, copy `canxium` to the `PATH`. A typical directory is `/usr/local/bin`.

```bash
sudo cp canxium /usr/local/bin
```
